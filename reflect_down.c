...
#define FIND_LOW(N) \
LOOP_INOUT static inline unsigned find_low_##N( \
...
{
  if (nverts_wanted == 2)
    return find_low_2(verts_wanted, verts_of_lows, lows_of_verts_offsets,
        lows_of_verts);
  else
    return find_low_3(verts_wanted, verts_of_lows, lows_of_verts_offsets,
        lows_of_verts);
}
...
LOOP_KERNEL(reflect_down_entity,
  ...
  for (unsigned j = 0; j < lows_per_high; ++j) {
    ...
    lows_of_high[j] = find_low(verts_per_low, verts_wanted,
