...
void copy_tags(struct tags* a, struct tags* b, unsigned n)
{
  for (unsigned i = 0; i < count_tags(a); ++i) {
    ...
    void* data = 0;
    switch (t->type) {
      case TAG_U8:  data = copy_array(t->d.u8, n * t->ncomps);
                    break;
      case TAG_U32: data = copy_array(t->d.u32, n * t->ncomps);
                    break;
      case TAG_U64: data = copy_array(t->d.u64, n * t->ncomps);
                    break;
      case TAG_F64: data = copy_array(t->d.f64, n * t->ncomps);
                    break;
    };
