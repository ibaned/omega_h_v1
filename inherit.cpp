    ...
    unsigned ngen[4])
{
  for (unsigned i = 0; i < 4; ++i) {
    if (ndoms[i] && prods_of_doms_offsets[i])
      ngen[i] = array_at(prods_of_doms_offsets[i], ndoms[i]);
    else
      ngen[i] = 0;
  }
}
    ...
    unsigned* gen_data[4])
{
  for (unsigned i = 0; i < 4; ++i) {
    if (ndoms[i] && dom_data[i] && prods_of_doms_offsets[i])
      gen_data[i] = expand_array(dom_data[i],
          prods_of_doms_offsets[i], ndoms[i], width);
    else
      gen_data[i] = 0;
  }
}
    ...
    unsigned** p_verts_of_ents_out)
{
  ...
  unsigned nsame_ents = array_at(offset_of_same_ents, nents);
